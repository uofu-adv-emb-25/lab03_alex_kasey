...
void deadlock_thread(void *args)
{
    ...
    printf("Starting deadlock from thread: %s", arguments->thread_name);
    // Acquire first lock
    xSemaphoreTake(arguments->first_lock, portMAX_DELAY);
    {
        arguments->count++;
        printf("Inside first lock from thread: %s", arguments->thread_name);
        vTaskDelay(1000);
        // Acquire second lock
        xSemaphoreTake(arguments->second_lock, portMAX_DELAY);
        {
            arguments->count++;
            printf("Inside second lock from thread: %s", arguments->thread_name);
        }
